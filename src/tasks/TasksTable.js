...
import { GROUP_BY_TASKS, TASK_STATUS, TASK_PRIORITY } from '../config/constants'
...
class TasksTable extends Component {
    ...
    sortByProperty(array, property, order = SORT_AS_TYPES.ASC) {
        ...
        if (property === COLUMN_NAMES.priority) {
            ...
            if (order === SORT_AS_TYPES.ASC) {
                if (group[TASK_PRIORITY.high]) {
                    sortedList.push(...group[TASK_PRIORITY.high])
                }
                if (group[TASK_PRIORITY.medium]) {
                    sortedList.push(...group[TASK_PRIORITY.medium])
                }
                if (group[TASK_PRIORITY.low]) {
                    sortedList.push(...group[TASK_PRIORITY.low])
                }
                if (group[TASK_PRIORITY.none]) {
                    sortedList.push(...group[TASK_PRIORITY.none])
                }
            } else {
                if (group[TASK_PRIORITY.none]) {
                    sortedList.push(...group[TASK_PRIORITY.none])
    ...
    updateCurrentStatus = (index) => {
        ...
        if (currentStatus === TASK_STATUS.pending) {
            tasksList[index]['currentState'] = TASK_STATUS.completed
        } else {
            tasksList[index]['currentState'] = TASK_STATUS.pending
        }

        // Binding this reference as it will not be available in callback function
        this.setState.bind(this)({
            tasksList
        }, () => {
    ...
    getTableElements = (tasksList) => {
        return tasksList.map((task, index) => {
            ...
            if (task.currentState === TASK_STATUS.pending) {
    ...
    getTableHeader = () => {
        ...
        return (
            <tr>
                ...
                <th>Due By <button className="btn btn-primary pull-right" onClick={() => this.handleSorting(COLUMN_NAMES.dueDate)}>{ sortElementObj.dueDate }</button></th>
    ...
    render() {
        ...
        return (
            <React.Fragment>
                ...
                <TasksManage {...this.state.selectedTask} modalId="updateTaskModal" saveModalDetails={this.saveModalDetails} />
            </React.Fragment>
